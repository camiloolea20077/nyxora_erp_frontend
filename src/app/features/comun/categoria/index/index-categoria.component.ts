...
import { FormCategoriaComponent } from '../form/form-categoria.component';
...
export class IndexCategoriaComponent {
  ...
  readonly rows = signal<CategoriaTableModel[]>([]);
  ...
  readonly editing = signal<CategoriaModel | null>(null);
  ...
  async load(): Promise<void> {
    ...
    try {
      ...
      this.rows.set([]);
      this.alert.error('No se pudo cargar el listado de categorías');
    } finally {
  ...
  async editar(row: CategoriaTableModel): Promise<void> {
    try {
      const res = await lastValueFrom(this.service.getById(row.id));
      this.editing.set(res.data);
      this.showForm.set(true);
    } catch {
      this.alert.error('No se pudo abrir la categoría');
    }
  }

  eliminar(row: CategoriaTableModel): void {
