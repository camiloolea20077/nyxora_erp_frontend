...
import { OrdenCompraTableModel } from '../models/orden-compra.model';
...
@Component({
  ...
  styleUrl: './index-orden-compra.component.css',
})
export class IndexOrdenCompraComponent {
  ...
  readonly rows = signal<OrdenCompraTableModel[]>([]);
  readonly total = signal(0);
  readonly loading = signal(false);
  readonly page = signal(0);
  readonly pageSize = signal(10);
  search = '';
  private readonly proveedores = signal<Map<number, string>>(new Map());
  ...
  estadoSeverity(estado: string): TagSeverity {
    ...
    if (estado === 'recibida') return 'info';
    return 'secondary';
  }
  proveedor(id: number | null): string {
    if (id == null) return '';
    return this.proveedores().get(id) ?? `#${id}`;
  }
  ...
  async load(): Promise<void> {
    ...
    try {
      ...
      void this.resolverProveedores(res.data.content.map((r) => r.proveedorId));
    } catch {
      this.rows.set([]);
      this.alert.error('No se pudo cargar el listado de órdenes');
    } finally {
      this.loading.set(false);
    }
  }

  private async resolverProveedores(ids: (number | null)[]): Promise<void> {
    const map = new Map(this.proveedores());
  ...
  nuevo(): void {
    void this.router.navigate(['/ordenes-compra/nuevo']);
  }
  ver(row: OrdenCompraTableModel): void {
    void this.router.navigate(['/ordenes-compra', row.id]);
  }
  editar(row: OrdenCompraTableModel): void {
    void this.router.navigate(['/ordenes-compra', row.id, 'editar']);
  }

  async aprobar(row: OrdenCompraTableModel): Promise<void> {
    try {
      await lastValueFrom(this.service.aprobar(row.id));
  ...
  anular(row: OrdenCompraTableModel): void {
    this.confirm.confirm({
      message: `¿Anular la orden ${row.numero ?? row.id}?`,
      header: 'Confirmar',
  ...
  eliminar(row: OrdenCompraTableModel): void {
    this.confirm.confirm({
      message: `¿Eliminar la orden ${row.numero ?? row.id}?`,
