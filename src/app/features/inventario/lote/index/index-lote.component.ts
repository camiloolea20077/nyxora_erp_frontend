...
import { LoteModel, LoteTableModel } from '../models/lote.model';
...
export class IndexLoteComponent {
  ...
  readonly rows = signal<LoteTableModel[]>([]);
  ...
  readonly editing = signal<LoteModel | null>(null);
  ...
  async load(): Promise<void> {
    ...
    try {
      ...
      this.rows.set([]);
      this.alert.error('No se pudo cargar el listado de lotes');
    } finally {
  ...
  async editar(row: LoteTableModel): Promise<void> {
    try {
      const res = await lastValueFrom(this.service.getById(row.id));
      this.editing.set(res.data);
      this.showForm.set(true);
    } catch {
      this.alert.error('No se pudo abrir el lote');
    }
  }

  eliminar(row: LoteTableModel): void {
    this.confirm.confirm({
      message: `¿Eliminar el lote "${row.codigo}"?`,
  ...
  private async doDelete(id: number): Promise<void> {
    try {
      ...
      this.alert.success('Lote eliminado');
      this.load();
    } catch {
      this.alert.error('No se pudo eliminar el lote');
    }
  }
