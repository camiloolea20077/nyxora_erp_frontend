...
import { MarcaModel, MarcaTableModel } from '../models/marca.model';
import { FormMarcaComponent } from '../form/form-marca.component';
...
export class IndexMarcaComponent {
  ...
  readonly rows = signal<MarcaTableModel[]>([]);
  ...
  readonly editing = signal<MarcaModel | null>(null);
  ...
  async load(): Promise<void> {
    ...
    try {
      ...
      this.rows.set([]);
      this.alert.error('No se pudo cargar el listado de marcas');
    } finally {
  ...
  async editar(row: MarcaTableModel): Promise<void> {
    try {
      const res = await lastValueFrom(this.service.getById(row.id));
      this.editing.set(res.data);
      this.showForm.set(true);
    } catch {
      this.alert.error('No se pudo abrir la marca');
    }
  }

  eliminar(row: MarcaTableModel): void {
  ...
  private async doDelete(id: number): Promise<void> {
    try {
      ...
      this.alert.error('No se pudo eliminar la marca');
    }
  }
