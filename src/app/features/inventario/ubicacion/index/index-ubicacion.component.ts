...
import { FormUbicacionComponent } from '../form/form-ubicacion.component';

@Component({
  selector: 'app-index-ubicacion',
  standalone: true,
  ...
  templateUrl: './index-ubicacion.component.html',
  styleUrl: './index-ubicacion.component.css',
})
export class IndexUbicacionComponent {
  private readonly service = inject(UbicacionService);
  private readonly alert = inject(AlertService);
  private readonly confirm = inject(ConfirmationService);

  readonly rows = signal<UbicacionTableModel[]>([]);
  ...
  readonly editing = signal<UbicacionModel | null>(null);
  ...
  async load(): Promise<void> {
    ...
    try {
      ...
      this.rows.set([]);
      this.alert.error('No se pudo cargar el listado de ubicaciones');
    } finally {
  ...
  async editar(row: UbicacionTableModel): Promise<void> {
    try {
      const res = await lastValueFrom(this.service.getById(row.id));
      this.editing.set(res.data);
      this.showForm.set(true);
    } catch {
      this.alert.error('No se pudo abrir la ubicación');
    }
  }

  eliminar(row: UbicacionTableModel): void {
  ...
  private async doDelete(id: number): Promise<void> {
    try {
      ...
      this.alert.success('Ubicación eliminada');
      this.load();
    } catch {
      this.alert.error('No se pudo eliminar la ubicación');
    }
  }
