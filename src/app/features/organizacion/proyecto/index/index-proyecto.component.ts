...
import { ProyectoModel, ProyectoTableModel } from '../models/proyecto.model';
...
@Component({
  ...
  styleUrl: './index-proyecto.component.css',
})
export class IndexProyectoComponent {
  private readonly service = inject(ProyectoService);
  private readonly alert = inject(AlertService);
  private readonly confirm = inject(ConfirmationService);

  readonly rows = signal<ProyectoTableModel[]>([]);
  ...
  readonly editing = signal<ProyectoModel | null>(null);
  ...
  async load(): Promise<void> {
    ...
    try {
      ...
      this.rows.set([]);
      this.alert.error('No se pudo cargar el listado de proyectos');
    } finally {
  ...
  async editar(row: ProyectoTableModel): Promise<void> {
    try {
      const res = await lastValueFrom(this.service.getById(row.id));
      this.editing.set(res.data);
      this.showForm.set(true);
    } catch {
      this.alert.error('No se pudo abrir el proyecto');
    }
  }

  eliminar(row: ProyectoTableModel): void {
