...
import { FormCuentaComponent } from '../form/form-cuenta.component';
...
@Component({
  ...
  styleUrl: './index-cuenta.component.css',
})
export class IndexCuentaComponent {
  ...
  readonly rows = signal<CuentaTableModel[]>([]);
  ...
  async load(): Promise<void> {
    ...
    try {
      ...
      this.rows.set([]);
      this.alert.error('No se pudo cargar el plan de cuentas');
    } finally {
  ...
  async editar(row: CuentaTableModel): Promise<void> {
    try {
      const res = await lastValueFrom(this.service.getById(row.id));
      this.editing.set(res.data);
      this.showForm.set(true);
    } catch {
      this.alert.error('No se pudo abrir la cuenta');
    }
  }

  eliminar(row: CuentaTableModel): void {
    this.confirm.confirm({
      message: `¿Eliminar la cuenta "${row.nombreCuenta}"?`,
